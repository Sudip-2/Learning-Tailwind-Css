...
app.get('/jokes/:id', (req, res) => {
  try {
    const reqnum = parseInt(req.params.id)
    const joke = jokes.find((joke) => {
      return joke.id == reqnum
    })
    res.json(joke)
  }
  catch (error) {
...
app.get('/filter',(req,res) => {
  try{
    ...
    const passedArray = jokes.filter((joke) => {
      return joke.jokeType == query
    })
    res.json(passedArray)
  }
  catch(error){
    res.send(error.message)
  }
...
app.put('/jokes/:id',(req,res) => {
  ...
  const searchIndex = jokes.findIndex((joke) => {
    return joke.id === idparam
  })
  jokes[searchIndex] = replJoke
...
app.delete('/jokes/:id',(req,res) => {
  const idparam = parseInt(req.params.id)
  const index = jokes.findIndex((joke) => joke.id === idparam)
